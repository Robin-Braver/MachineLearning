'''编程实现累计BP算法，在西瓜数据集3.0上训练一个单隐层网络。通过可视化输出曲线图，和标准BP算法进行比较与分析'''
import numpy as np
...
# 累积BP算法
def ABP_NN(data, q=10, lr=0.1,err=0.01,Epoch=200):
    """
    ABP_NN:标准反向传播算法
    :param data: 带有标签的数据集
    :param q:隐层神经元个数
    ...
    for epoch in range(Epoch):
        ...
        for i in range(num_sample):  # 由于标准bp需要每个样本更新参数，因此需要逐样本训练
            ...
            g = y_cal * (1 - y_cal) * (Y[i, :] - y_cal)  # 计算5.10 gj  1*l array
...
def splitDataSet(data,test_size=0.3,seed=None):
    """
...
train_data,test_data = splitDataSet(Data,0.5)
v,w = ABP_NN(data=train_data,q=15,lr=0.1,err=0.001,Epoch=1000)
test(test_data,v, w)
