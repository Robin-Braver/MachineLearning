...
class DecisionTree(object):
    ...
    def __init__(self, data_set, data_test, feature_dict):
        self.root = self.tree_generate(data_set, data_test, feature_dict)
        self.data_set = data_set
    ...
    def tree_generate(self, data_set, data_test, feature_dict):
        ...
        for feat in feat_set:
            ...
            if sub_data_set.num == 0:   # 如果子数据集为空
                ...
                new_node = self.tree_generate(sub_data_set,data_test.group_feature(best_feature_index, feat), feature_dict)
        ...
        for feat in feat_set_full-feat_set:
            ...
            new_node.parent = node  # 设置父节点
            node.children.append(new_node)
        if self.pre_test(node, data_test) <= DecisionTree.pre_test_majority(data_set.get_mostLabel(), data_test): #如果剪枝后性能下降或者性能不变就不剪枝
            return node
        node = Node()
        node.isLeaf = True  # 标记为叶子节点
        node.label = data_set.get_mostLabel()  # 标记叶子节点的类别为数据集中样本数最多的类
