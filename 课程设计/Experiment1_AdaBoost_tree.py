...
def adaBoostTrain(X, y, tree_num=3,max_depth=2):
    ...
    for _ in range(tree_num):
        tree = createSingleTree(X, y, D,max_depth=max_depth)#生成一个决策树
        hx = predictBase(tree, X)
        err_rate = np.sum(D[hx != y])#计算误差
...
def pltAdaBoostDecisionBound(X_train, y_train, X_test,y_test,trees, a):
    pred = adaBoostPredict(X_test,trees,a)
    correct = 0
    for i in range(len(pred)):
        if pred[i] == y_test[i]:
            correct += 1
    acc = "---测试集正确率为："+str(correct)+'/'+str(len(pred))+'='+str(correct*1./len(pred))
    pos_train = y_train == 1
    neg_train = y_train == -1
    pos_test = y_test == 1
    neg_test = y_test == -1
    x_tmp = np.linspace(0, 0.8, 200)
    y_tmp = np.linspace(0, 0.6, 200)
    ...
    plt.scatter(X_train[pos_train, 0], X_train[pos_train, 1], marker='+', label='训练样本-好瓜', color='k')
    plt.scatter(X_train[neg_train, 0], X_train[neg_train, 1], marker='_', label='训练样本-坏瓜', color='k')
    plt.scatter(X_test[pos_test, 0], X_test[pos_test, 1], marker='+', label='测试样本-好瓜', color='g')
    plt.scatter(X_test[neg_test, 0], X_test[neg_test, 1], marker='_', label='测试样本-坏瓜', color='g')
...
if __name__ == "__main__":
    data = pd.read_csv('data\西瓜数据集3a.txt', sep=' ',header=None)
    X = data.iloc[:, :2].values
    y = data.iloc[:, 2].values
    y[y == 0] = -1

    train_data,train_label,test_data,test_label = utils.splitDataSet(X,y,0.3,2012)


    trees, a = adaBoostTrain(train_data, train_label,tree_num=1)
    pltAdaBoostDecisionBound(train_data,train_label,test_data,test_label, trees, a)
    trees, a = adaBoostTrain(train_data, train_label,tree_num=3)
    pltAdaBoostDecisionBound(train_data, train_label, test_data, test_label, trees, a)
    trees, a = adaBoostTrain(train_data, train_label,tree_num=5)
    pltAdaBoostDecisionBound(train_data, train_label, test_data, test_label, trees, a)
    trees, a = adaBoostTrain(train_data, train_label,tree_num=7)
    pltAdaBoostDecisionBound(train_data,train_label,test_data,test_label, trees, a)
    trees, a = adaBoostTrain(train_data, train_label,tree_num=11)
    pltAdaBoostDecisionBound(train_data, train_label, test_data, test_label, trees, a)
    trees, a = adaBoostTrain(train_data, train_label, tree_num=13)
    pltAdaBoostDecisionBound(train_data, train_label, test_data, test_label, trees, a)